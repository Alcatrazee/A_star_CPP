...
void Expend_current_point(int cost_map[Height][Height], int current_point[2], int Openset[array_size][6], int ClosedSet[array_size][6], int end_point[2], int start_point[2],int closed_list_counter){
	...
	point[0] = current_point[0];
	point[1] = current_point[1] + 1;
	if (current_point[1] < Height&& !Is_in_open_or_closed_set(point, Openset, ClosedSet, closed_list_counter) && cost_map[point[0]][point[1]] != 100){
		int index = 0;
		cost_map[point[0]][point[1]] = Get_heuristic_function(point, end_point) + Get_gn(point, start_point);
		for (int i = 0; i < array_size; i++){
			if (Openset[i][valid_bit] == 0){
...
void Draw_map_with_path(int  show_map[Height][Width], int Path[length][2],int Path_length){
	for (int i = 0; i < Path_length; i++){
		show_map[Path[i][coorx]][Path[i][coory]] = 1;
	}
}

int main(int argc, char **argv)
{
  int map[Height][Width], cost_map[Height][Width], show_map[Height][Width];
  int Path[length][2];
  int Path_index=0;
  Create_map(map);
  Create_map(cost_map);
